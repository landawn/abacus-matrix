 ...
 * a validated backing array. The {@link #ShortMatrix(short[][]) constructor} and {@link #wrap(short[][]) wrap(...)}
 ...
 * independently owned matrix. {@link #copyOf(short[][])}, conversions, and mapping operations do not
 * share mutable cell storage with a non-empty source; operations producing an empty matrix may return
 * the shared empty singleton.</p>
 *
...
public final class ShortMatrix extends AbstractMatrix<short[], ShortList, ShortStream, Stream<ShortStream>, ShortMatrix> {
     ...
     *
     * <p>For a non-empty input, unlike {@link #wrap(short[][])}, which wraps the caller's array without copying, this factory clones
    ...
    public <E extends Exception> void updateColumn(final int columnIndex, final Throwables.ShortUnaryOperator<E> operator)
        ...
        forEachDistinctRow(row -> row[columnIndex] = operator.applyAsShort(row[columnIndex]));
    }
     ...
     * ShortMatrix nonSquare = ShortMatrix.wrap(new short[][] {{1, 2, 3}, {4, 5, 6}});
    ...
    public <E extends Exception> void updateAll(final Throwables.ShortUnaryOperator<E> operator) throws IllegalArgumentException, E {
        ...
        if (Matrices.shouldRunInParallel(this)) {
            if (hasAliasedRows()) {
                ...
                final Throwables.IntBiConsumer<E> elementAction = (i, j) -> distinctRows[i][j] = operator.applyAsShort(distinctRows[i][j]);
     ...
     * {@link Arrays#mutateFlattened(short[][], Throwables.Consumer)} for the exact semantics of the
     ...
     *
     * ShortMatrix.empty().mutateFlattened(arr -> { });  // no-op on empty matrix
     * }</pre>
     *
     ...
     * @see Arrays#mutateFlattened(short[][], Throwables.Consumer)
     */
    @Override
     ...
     * }</pre>
     *
     * @return a ShortStream of main-diagonal elements, or an empty stream if this is the empty 0x0 matrix
     * @throws IllegalStateException if the matrix is not square (rowCount != columnCount)
     */
    @Override
     ...
     * }</pre>
     *
     * @return a ShortStream of anti-diagonal elements, or an empty stream if this is the empty 0x0 matrix
     * @throws IllegalStateException if the matrix is not square (rowCount != columnCount)
     */
    @Override
