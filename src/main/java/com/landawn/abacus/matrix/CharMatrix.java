...
public final class CharMatrix extends AbstractMatrix<char[], CharList, CharStream, Stream<CharStream>, CharMatrix> {
    ...
    /** The shared {@code 0 x 0} char matrix returned by {@link #empty()} and public empty-array factories. */
     ...
     * }</pre>
     *
     * @param a the two-dimensional char array whose rows are shared, must not be {@code null}
     ...
     */
    public CharMatrix(final char[][] a) throws IllegalArgumentException {
    ...
    /**
     * Wraps the supplied two-dimensional char array as {@code CharMatrix}.
     *
     ...
     *
     * CharMatrix.wrap((char[][]) null);          // throws IllegalArgumentException
     * CharMatrix.wrap(new char[0][0]).isEmpty(); // returns true
     * }</pre>
     *
     ...
     *         different lengths (i.e. the array is not rectangular), or if two positions reference the same row array
     */
    public static CharMatrix wrap(final char[]... a) throws IllegalArgumentException {
     ...
     *
     * <p>For a non-empty input, unlike {@link #wrap(char[][])}, which copies the outer array but shares the caller's rows,
     ...
     * @see #copy()
     */
    public static CharMatrix copyOf(final char[]... a) throws IllegalArgumentException {
        N.checkArgNotNull(a, "Matrix array cannot be null");
     ...
     * @see #random(int, int)
     */
    public static CharMatrix randomRow(final int columnCount) throws IllegalArgumentException {
     ...
     * CharMatrix.random(0, 3).columnCount(); // returns 3 (shape is preserved)
     * }</pre>
     *
     ...
     */
    public static CharMatrix random(final int rowCount, final int columnCount) throws IllegalArgumentException {
        return random(rowCount, columnCount, defaultRandomGenerator());
    }

    /**
     * Creates a matrix of random UTF-16 code units using the caller-supplied random source.
     *
    ...
    public static CharMatrix random(final int rowCount, final int columnCount, final RandomGenerator randomGenerator) throws IllegalArgumentException {
        ...
        for (char[] ea : a) {
            for (int i = 0; i < columnCount; i++) {
                ea[i] = (char) randomGenerator.nextInt(BOUND);
            }
        }
     ...
     */
    public static CharMatrix range(final char startInclusive, final char endExclusive, final int step) throws IllegalArgumentException {
     ...
     * @throws IllegalArgumentException if {@code step} is zero
     */
    public static CharMatrix rangeClosed(final char startInclusive, final char endInclusive, final int step) throws IllegalArgumentException {
     ...
     */
    public static CharMatrix ofMainDiagonal(final char[] mainDiagonal) throws IllegalArgumentException {
     ...
     * @see #ofDiagonals(char[], char[])
     */
    public static CharMatrix ofAntiDiagonal(final char[] antiDiagonal) throws IllegalArgumentException {
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * Matrix<Character> boxedMatrix = Matrix.wrap(new Character[][] {{'a', 'b'}, {null, 'c'}});
     ...
     * @see #boxed()
     */
    public static CharMatrix unbox(final Matrix<Character> x) throws IllegalArgumentException {
     ...
     *
     * CharMatrix single = CharMatrix.wrap(new char[][] {{'x'}});
     * single.mainDiagonalCopy();   // returns ['x']
     *
     * CharMatrix nonSquare = CharMatrix.wrap(new char[][] {{'a', 'b', 'c'}, {'d', 'e', 'f'}});
     * nonSquare.mainDiagonalCopy(); // returns ['a', 'e']
     * }</pre>
     *
     * @return a new char array containing a copy of the main diagonal elements
     */
    @Override
    public char[] mainDiagonalCopy() {
        final int diagonalLength = diagonalLength();
        final char[] res = new char[diagonalLength];
     ...
     *
     * CharMatrix nonSquare = CharMatrix.wrap(new char[][] {{'a', 'b', 'c'}, {'d', 'e', 'f'}});
     * nonSquare.setMainDiagonal(new char[] {'x', 'y'}); // supported for a 2x3 matrix
     * }</pre>
     *
     ...
     * CharMatrix nonSquare = CharMatrix.wrap(new char[][] {{'a', 'b', 'c'}, {'d', 'e', 'f'}});
     * nonSquare.updateMainDiagonal(c -> c); // supported for a rectangular matrix
     * }</pre>
     *
     * @param <E> the type of exception that the operator may throw
     ...
     *
     * CharMatrix nonSquare = CharMatrix.wrap(new char[][] {{'a', 'b', 'c'}, {'d', 'e', 'f'}});
     * nonSquare.setAntiDiagonal(new char[] {'x', 'y'}); // supported for a 2x3 matrix
     * }</pre>
     *
     ...
     * CharMatrix nonSquare = CharMatrix.wrap(new char[][] {{'a', 'b', 'c'}, {'d', 'e', 'f'}});
     * nonSquare.updateAntiDiagonal(c -> c); // supported for a rectangular matrix
     * }</pre>
     *
     * @param <E> the type of exception that the operator may throw
     ...
     *
     * CharMatrix shift = CharMatrix.wrap(new char[][] {{'a', 'b'}});
     ...
     *
     * CharMatrix diag = CharMatrix.wrap(new char[2][2]);
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * CharMatrix matrix = CharMatrix.wrap(new char[][] {{'a', 'e', 'c'}, {'b', 'f', 'd'}});
     * matrix.replaceIf(c -> c < 'd', 'x');
     * matrix.rowView(0);          // returns ['x', 'e', 'x']
     * matrix.rowView(1);          // returns ['x', 'f', 'd']
     *
     * CharMatrix none = CharMatrix.wrap(new char[][] {{'a', 'b'}});
     ...
     *
     * CharMatrix firstCol = CharMatrix.wrap(new char[][] {{'a', 'b'}, {'c', 'd'}});
     ...
     *
     * <p>The operation may be performed in parallel for large matrices to improve performance. If parallelized, the supplied function must be thread-safe.
     * This is the non-mutating counterpart to {@link #updateAll(Throwables.CharUnaryOperator)}.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * CharMatrix matrix = CharMatrix.wrap(new char[][] {{'a', 'b'}, {'c', 'd'}});
     ...
     *
     * CharMatrix single = CharMatrix.wrap(new char[][] {{'a'}});
     * single.fill('z');
     ...
     */
    public void copyFrom(final char[][] source) throws IllegalArgumentException {
        copyFrom(0, 0, source);
    }
     ...
     */
    @Override
    public CharMatrix resize(final int newRowCount, final int newColumnCount) throws IllegalArgumentException {
     ...
     * <p>The original matrix is never modified; the result has independent cell storage.</p>
     *
     * <p><b>Comparison with {@link #pad(int, int, int, int, char)}:</b>
     * {@code resize} takes <em>absolute</em> target dimensions and may truncate existing content.
     * {@code pad} takes <em>relative</em> padding amounts per edge and <em>never truncates</em>.
     * Use {@code pad} when the entire original content must be preserved.</p>
     *
     ...
     * CharMatrix bordered = matrix.pad(1, 1, 1, 1);
     ...
     * CharMatrix shifted = matrix.pad(0, 0, 2, 0);
     * // Result: [[(char) 0, (char) 0, 'a', 'b'],
     * //          [(char) 0, (char) 0, 'c', 'd']]
     *
     * matrix.pad(0, 0, 0, 0).equals(matrix); // returns true (no padding -> equal copy)
     ...
     */
    @Override
    public CharMatrix pad(final int padTop, final int padBottom, final int padLeft, final int padRight) throws IllegalArgumentException {
        return pad(padTop, padBottom, padLeft, padRight, CHAR_0);
    }
     ...
     * CharMatrix bordered = matrix.pad(1, 1, 1, 1, 'x');
     * // Result: [['x', 'x', 'x', 'x'],
     * //          ['x', 'a', 'b', 'x'],
     * //          ['x', 'c', 'd', 'x'],
     * //          ['x', 'x', 'x', 'x']]
     ...
     * //          [' ', ' ', 'c', 'd']]
     *
     * matrix.pad(0, 0, 0, 0, 'x').equals(matrix); // returns true (no padding -> equal copy)
     * matrix.pad(0, -1, 0, 0, 'x');               // throws IllegalArgumentException (negative padding)
     * }</pre>
     *
     * @param padTop number of padding rows to add above the original matrix; must be {@code >= 0}
     * @param padBottom number of padding rows to add below the original matrix; must be {@code >= 0}
     ...
     * @see #resize(int, int, char)
     */
    public CharMatrix pad(final int padTop, final int padBottom, final int padLeft, final int padRight, final char defaultValue)
     ...
     *
     * CharMatrix single = CharMatrix.wrap(new char[][] {{'x', 'y'}});
     ...
     *
     * CharMatrix multi = CharMatrix.wrap(new char[][] {{'a', 'b'}, {'c', 'd'}});
     ...
     * CharMatrix wide = CharMatrix.wrap(new char[][] {{'a', 'b', 'c'}, {'d', 'e', 'f'}});
     ...
     * <pre>{@code
     * CharMatrix matrix = CharMatrix.wrap(new char[][] {{'a', 'b', 'c'}, {'d', 'e', 'f'}});
     * CharMatrix reshaped = matrix.reshapeAndPad(3, 2);
     * reshaped.rowView(0);                    // returns ['a', 'b'] -> {{'a', 'b'}, {'c', 'd'}, {'e', 'f'}}
     * reshaped.get(2, 1);                     // returns 'f'
     *
     * CharMatrix extended = matrix.reshapeAndPad(2, 4);
    ...
    @Override
    public CharMatrix reshapeAndPad(final int newRowCount, final int newColumnCount) throws IllegalArgumentException {
        N.checkArgument(newRowCount >= 0, MSG_NEGATIVE_DIMENSION, cs.newRowCount, newRowCount);
        N.checkArgument(newColumnCount >= 0, MSG_NEGATIVE_DIMENSION, cs.newColumnCount, newColumnCount);
        checkNonNegativeShape(newRowCount, newColumnCount);
     ...
     *
     * CharMatrix single = CharMatrix.wrap(new char[][] {{'x', 'y', 'z'}});
     ...
     */
    @Override
    public CharList flatten() throws IllegalStateException {
        // Check for overflow before allocation
     ...
     */
    @Override
    public <E extends Exception> void mutateViaFlatArray(final Throwables.Consumer<? super char[], E> action)
     ...
     *
     * CharMatrix wrong = CharMatrix.wrap(new char[][] {{'x'}});
     ...
     * CharMatrix one = CharMatrix.wrap(new char[][] {{1}});
     * int wrappedValue = max.add(one).get(0, 0); // 0 (wrapped)
     *
     * CharMatrix wrong = CharMatrix.wrap(new char[][] {{'x', 'y', 'z'}});
     ...
     * CharMatrix zero = CharMatrix.wrap(new char[][] {{0}});
     * CharMatrix one = CharMatrix.wrap(new char[][] {{1}});
     ...
     *
     * CharMatrix wrong = CharMatrix.wrap(new char[][] {{1, 2, 3}}); // 1x3, this.columnCount is 2
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * CharMatrix primitive = CharMatrix.wrap(new char[][] {{'a', 'b'}, {'c', 'd'}});
    ...
    public Matrix<Character> boxed() {
        ...
        return new Matrix<>(c, Character.class, columnCount);
    }
     ...
     *
     * CharMatrix.wrap(new char[][] {{'0'}}).toIntMatrix().get(0, 0); // returns 48 (code unit of '0')
     ...
     *
     * CharMatrix.wrap(new char[][] {{'0'}}).toLongMatrix().get(0, 0); // returns 48L
     ...
     *
     * CharMatrix.wrap(new char[][] {{'0'}}).toFloatMatrix().get(0, 0); // returns 48.0f
     ...
     *
     * CharMatrix.wrap(new char[][] {{'0'}}).toDoubleMatrix().get(0, 0); // returns 48.0
     ...
     * CharMatrix nonSquare = CharMatrix.wrap(new char[][] {{'a', 'b', 'c'}, {'d', 'e', 'f'}});
     * nonSquare.mainDiagonalStream().toArray(); // returns ['a', 'e']
     * }</pre>
     *
     ...
     * CharMatrix nonSquare = CharMatrix.wrap(new char[][] {{'a', 'b', 'c'}, {'d', 'e', 'f'}});
     * nonSquare.antiDiagonalStream().toArray(); // returns ['c', 'e']
     * }</pre>
     *
     ...
     * CharMatrix.wrap(new char[][] {{'x'}}).rowMajorStream().sum(); // returns 120 (single element)
     * }</pre>
     *
     ...
     * CharMatrix.wrap(new char[][] {{'x'}}).columnMajorStream().sum(); // returns 120 (single element)
     * }</pre>
     *
     ...
     * CharMatrix.wrap(new char[][] {{'a'}, {'b'}}).columnStreams().count();  // returns 1 (single column)
     * }</pre>
     *
     ...
     * or {@link #updateAll(Throwables.CharUnaryOperator)} to modify this matrix in place.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * CharMatrix matrix = CharMatrix.wrap(new char[][] {{'a', 'b'}, {'c', 'd'}});
    ...
    /**
     * Renders one row per line using quoted, escaped UTF-16 code units. Control characters,
     * backslash, apostrophe, non-ASCII code units, and surrogate code units therefore cannot alter
     * the output layout or masquerade as delimiters. A zero-row matrix renders as {@code "[]"}.
     *
     * @return the unambiguous multi-line representation of this matrix
     */
    @Override
    ...
    private String escapedString(final boolean multiline) {
        ...
        try {
            ...
            for (int i = 0; i < rowCount; i++) {
                if (i > 0) {
                    sb.append(multiline ? ARRAY_PRINT_SEPARATOR : ", ");
                }
            ...
            // StringBuilder never throws IOException; this only satisfies Appendable's general contract.
        ...
        switch (value) {
            ...
            default:
                if (value >= 0x20 && value <= 0x7e) {
                    ...
                    // Render one UTF-16 code unit at a time. In particular, lone surrogates remain
                    // visible and valid surrogate pairs are never silently interpreted as code points.
                    output.append('\\').append('u');
                    final String hex = Integer.toHexString(value).toUpperCase(java.util.Locale.ROOT);
     ...
     *
     * CharMatrix m3 = CharMatrix.wrap(new char[][] {{'a', 'b'}, {'c', 'x'}});
    ...
    /**
     * Returns a compact diagnostic representation with every UTF-16 code unit quoted and escaped.
     * Printable ASCII characters appear directly; special characters and every other code unit use
     * Java-style escapes. For example, a newline is rendered as a backslash followed by {@code n},
     * and a surrogate is rendered as a four-digit hexadecimal escape.
     *
     * @return an unambiguous two-dimensional representation of this matrix
     * @see #println()
     */
    @Override
