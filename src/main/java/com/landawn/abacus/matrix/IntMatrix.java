 ...
 * validated backing array. The constructor and {@link #wrap(int[]...)} snapshot the outer array
 ...
 * {@link #copyOf(int[]...)}, conversions, and mapping operations do not share mutable cell storage.
 ...
 * around modulo 2<sup>32</sup>. Use the corresponding {@code *Widened} methods to retain wider values or
 * {@code *Exact} methods to report overflow.</p>
 *
...
public final class IntMatrix extends AbstractMatrix<int[], IntList, IntStream, Stream<IntStream>, IntMatrix> {
    ...
    /** The shared {@code 0 x 0} int matrix returned by {@link #empty()} and factories producing a {@code 0 x 0} result. */
    private static final IntMatrix EMPTY_INT_MATRIX = new IntMatrix(new int[0][0]);

    /**
     * Constructs an {@code IntMatrix} that shares the supplied row arrays.
     *
     ...
     * }</pre>
     *
     * @param a the two-dimensional int array whose rows are shared; must not be {@code null}
     ...
     */
    IntMatrix(final int[][] a, final int columnCount) throws IllegalArgumentException {
        super(N.checkArgNotNull(a, "Matrix array cannot be null"), int.class, columnCount);
    }
     ...
     */
    private IntMatrix(final int[][] a, final int columnCount, final boolean rowsAreKnownDistinct) throws IllegalArgumentException {
        super(N.checkArgNotNull(a, "Matrix array cannot be null"), int.class, columnCount, rowsAreKnownDistinct);
    }
     ...
     */
    static IntMatrix wrapResult(final int[][] a, final int columnCount) throws IllegalArgumentException {
     ...
     * }</pre>
     *
     * @return the shared empty {@code IntMatrix} singleton
     */
     ...
     *
     * <p>For a non-empty input, unlike {@link #wrap(int[][])}, which shares the caller's row arrays, this factory clones
     ...
     */
    public static IntMatrix randomRow(final int columnCount, final RandomGenerator randomGenerator) throws IllegalArgumentException, RuntimeException {
    ...
    /**
     * Creates a matrix whose values are drawn uniformly from the complete {@code int} range by
     ...
     * @return a new {@code IntMatrix} with primitive int values, with the same shape as {@code x}
     ...
     *
     * IntMatrix.empty().mainDiagonalCopy();   // returns []
     ...
     */
    @Override
    public int[] mainDiagonalCopy() {
        final int diagonalLength = diagonalLength();
        final int[] res = new int[diagonalLength];
     ...
     *
     * matrix.setMainDiagonal(new int[] {1}); // throws IllegalArgumentException (length != min(rowCount, columnCount))
     * IntMatrix rectangular = IntMatrix.wrap(new int[][] {{1, 2, 3}, {4, 5, 6}});
     * rectangular.setMainDiagonal(new int[] {7, 8}); // valid; updates two diagonal cells
     * }</pre>
     *
     * @param mainDiagonal the new values for the main diagonal; must be non-{@code null} and have the diagonal length
     * @throws IllegalArgumentException if {@code mainDiagonal} is {@code null}
     *         or its length differs from {@code min(rowCount, columnCount)}
     */
    @Override
     ...
     * IntMatrix rectangular = IntMatrix.wrap(new int[][] {{1, 2, 3}, {4, 5, 6}});
     * rectangular.updateMainDiagonal(x -> -x); // updates two diagonal cells
     * }</pre>
     *
     * @param <E> the type of exception that the operator may throw
     ...
     */
    public <E extends Exception> void updateMainDiagonal(final Throwables.IntUnaryOperator<E> operator) throws IllegalArgumentException, E {
        N.checkArgNotNull(operator, cs.operator);
     ...
     *
     * IntMatrix.empty().antiDiagonalCopy();   // returns []
     * IntMatrix rectangular = IntMatrix.wrap(new int[][] {{1, 2, 3}, {4, 5, 6}});
     ...
     *
     * matrix.setAntiDiagonal(new int[] {1}); // throws IllegalArgumentException (length != min(rowCount, columnCount))
     * IntMatrix rectangular = IntMatrix.wrap(new int[][] {{1, 2, 3}, {4, 5, 6}});
     ...
     * IntMatrix rectangular = IntMatrix.wrap(new int[][] {{1, 2, 3}, {4, 5, 6}});
     * rectangular.updateAntiDiagonal(x -> -x); // updates two anti-diagonal cells
     * }</pre>
     *
     * @param <E> the type of exception that the operator may throw
     ...
     */
    public <E extends Exception> void updateAntiDiagonal(final Throwables.IntUnaryOperator<E> operator) throws IllegalArgumentException, E {
        N.checkArgNotNull(operator, cs.operator);
     ...
     * big.copyFrom(new int[][] {{1, 2, 3}, {4, 5, 6}, {7, 8, 9}});
     * big.get(1, 1);                          // returns 5 (only overlapping region copied)
     *
     * matrix.copyFrom((int[][]) null);           // throws IllegalArgumentException (source is null)
     * }</pre>
     *
     ...
     *
     * matrix.copyFrom(0, 0, (int[][]) null);                 // throws IllegalArgumentException (source is null)
     * matrix.copyFrom(-1, 0, new int[][] {{1}});             // throws IndexOutOfBoundsException (destRowIndex < 0)
     * matrix.copyFrom(0, 5, new int[][] {{1}});              // throws IndexOutOfBoundsException (destColumnIndex > columnCount)
     * }</pre>
     *
    ...
    public IntMatrix resize(final int newRowCount, final int newColumnCount, final int defaultValue) throws IllegalArgumentException {
        ...
        if (newRowCount <= rowCount && newColumnCount <= columnCount) {
            ...
            return wrapResult(extendedData, newColumnCount);
        }
    }
     ...
     * }</pre>
     *
     * @return a new {@code IntMatrix} of shape {@code columnCount x rowCount}; an {@code N x 0}
     ...
     * @return a new {@code IntMatrix} with dimensions {@code (rowCount * rowRepeats) × (columnCount * columnRepeats)}
     ...
     * {@link Arrays#mutateViaFlatArray(int[][], Throwables.Consumer)} for the exact semantics of the
     ...
     *
     * IntMatrix.empty().mutateViaFlatArray(arr -> { });  // no-op on empty matrix
     * }</pre>
     *
     ...
     * wraps around modulo 2<sup>32</sup> on overflow.
     * To keep the wider intermediate value instead, use {@link #addWidened(IntMatrix)}; to be told
     * about an overflow rather than silently wrapping, use {@link #addExact(IntMatrix)}.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * @see #addWidened(IntMatrix)
     * @see #addExact(IntMatrix)
     */
    ...
    /**
     * Adds corresponding elements after widening both operands to {@code long}.
     *
     * @param other the matrix to add; must have the same shape
     * @return the exact element-wise sums in a {@link LongMatrix}
     * @throws IllegalArgumentException if {@code other} is {@code null} or has a different shape
     */
    public LongMatrix addWidened(final IntMatrix other) throws IllegalArgumentException {
        N.checkArgNotNull(other, cs.other);
        N.checkArgument(isSameShape(other), "Cannot add matrices with different shapes: this is {}x{} but other is {}x{}", rowCount, columnCount,
                other.rowCount, other.columnCount);
        ...
        for (int i = 0; i < rowCount; i++) {
            for (int j = 0; j < columnCount; j++) {
                result[i][j] = (long) a[i][j] + other.a[i][j];
            }
        }
    ...
    /**
     * Adds corresponding elements with {@link Math#addExact(int, int)}.
     *
     * @param other the matrix to add; must have the same shape
     * @return the exact element-wise sums
     * @throws IllegalArgumentException if {@code other} is {@code null} or has a different shape
     * @throws ArithmeticException if any sum overflows {@code int}
     */
    public IntMatrix addExact(final IntMatrix other) throws IllegalArgumentException, ArithmeticException {
        N.checkArgNotNull(other, cs.other);
        N.checkArgument(isSameShape(other), "Cannot add matrices with different shapes: this is {}x{} but other is {}x{}", rowCount, columnCount,
                other.rowCount, other.columnCount);
     ...
     * {@link #toLongMatrix()} first or use {@link #mapToLong(Throwables.IntToLongFunction)}.
     * To keep the wider intermediate value instead, use {@link #subtractWidened(IntMatrix)}; to be
     * told about an overflow rather than silently wrapping, use {@link #subtractExact(IntMatrix)}.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * @see #add(IntMatrix)
     * @see #zipWith(IntMatrix, Throwables.IntBinaryOperator)
     * @see #subtractWidened(IntMatrix)
     * @see #subtractExact(IntMatrix)
     */
    ...
    /**
     * Subtracts corresponding elements after widening both operands to {@code long}.
     *
     * @param other the matrix to subtract; must have the same shape
     * @return the exact element-wise differences in a {@link LongMatrix}
     * @throws IllegalArgumentException if {@code other} is {@code null} or has a different shape
     */
    public LongMatrix subtractWidened(final IntMatrix other) throws IllegalArgumentException {
        N.checkArgNotNull(other, cs.other);
        N.checkArgument(isSameShape(other), "Cannot subtract matrices with different shapes: this is {}x{} but other is {}x{}", rowCount, columnCount,
                other.rowCount, other.columnCount);
    ...
    /**
     * Subtracts corresponding elements with {@link Math#subtractExact(int, int)}.
     *
     * @param other the matrix to subtract; must have the same shape
     * @return the exact element-wise differences
     * @throws IllegalArgumentException if {@code other} is {@code null} or has a different shape
     * @throws ArithmeticException if any difference overflows {@code int}
     */
    public IntMatrix subtractExact(final IntMatrix other) throws IllegalArgumentException, ArithmeticException {
        N.checkArgNotNull(other, cs.other);
        N.checkArgument(isSameShape(other), "Cannot subtract matrices with different shapes: this is {}x{} but other is {}x{}", rowCount, columnCount,
                other.rowCount, other.columnCount);
     ...
     * {@link #toLongMatrix()} and multiply there.
     * To keep the wider intermediate value instead, use {@link #matrixMultiplyWidened(IntMatrix)};
     * to be told about an overflow rather than silently wrapping, use
     * {@link #matrixMultiplyExact(IntMatrix)}.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * IntMatrix a = IntMatrix.wrap(new int[][] {{1, 2}, {3, 4}});
     ...
     * @see #matrixMultiplyWidened(IntMatrix)
     * @see #matrixMultiplyExact(IntMatrix)
     */
     ...
     */
    public LongMatrix matrixMultiplyWidened(final IntMatrix other) throws IllegalArgumentException {
        N.checkArgNotNull(other, cs.other);
        N.checkArgument(columnCount == other.rowCount,
                "Matrix dimensions incompatible for multiplication: this is {}x{}, other is {}x{} (this.columnCount must equal other.rowCount)", rowCount,
                columnCount, other.rowCount, other.columnCount);
    ...
    /**
     * Multiplies this matrix by {@code other} using {@code long} arithmetic and reports overflow
     * during the left-to-right accumulation of any dot product.
     *
     * @param other the right-hand matrix
     * @return the matrix product in a {@link LongMatrix}
     * @throws IllegalArgumentException if {@code other} is {@code null} or the inner dimensions differ
     * @throws ArithmeticException if a dot-product sum overflows {@code long}
     */
    public LongMatrix matrixMultiplyWidenedExact(final IntMatrix other) throws IllegalArgumentException, ArithmeticException {
        N.checkArgNotNull(other, cs.other);
        N.checkArgument(columnCount == other.rowCount,
                "Matrix dimensions incompatible for multiplication: this is {}x{}, other is {}x{} (this.columnCount must equal other.rowCount)", rowCount,
                columnCount, other.rowCount, other.columnCount);
        ...
        for (int i = 0; i < rowCount; i++) {
            for (int k = 0; k < columnCount; k++) {
                ...
                for (int j = 0; j < other.columnCount; j++) {
                    result[i][j] = Math.addExact(result[i][j], left * other.a[k][j]);
                }
            }
        }
     ...
     */
    public IntMatrix matrixMultiplyExact(final IntMatrix other) throws IllegalArgumentException, ArithmeticException {
        // Accumulate the whole dot product before narrowing (matrixMultiplyWidenedExact already
        // validates the arguments). Checking each product and each partial sum in int instead would
        // incorrectly reject valid results whose positive and negative terms cancel.
        final LongMatrix widened = matrixMultiplyWidenedExact(other);
        final int[][] result = new int[rowCount][other.columnCount];

        for (int i = 0; i < rowCount; i++) {
            for (int j = 0; j < other.columnCount; j++) {
                result[i][j] = Math.toIntExact(widened.get(i, j));
            }
        }
    ...
    public Matrix<Integer> boxed() {
        ...
        return new Matrix<>(c, Integer.class, columnCount);
    }
    ...
    public LongMatrix toLongMatrix() {
        return rowCount == 0 ? LongMatrix.wrapResult(new long[0][], columnCount) : LongMatrix.from(a);
    }
    ...
    public FloatMatrix toFloatMatrix() {
        return rowCount == 0 ? FloatMatrix.wrapResult(new float[0][], columnCount) : FloatMatrix.from(a);
    }
    ...
    public Stream<IntStream> rowStreams(final int fromRowIndex, final int toRowIndex) throws IndexOutOfBoundsException {
        ...
        return Stream.of(new ObjIteratorEx<>() {
             ...
             */
            @Override
            public IntStream next() throws NoSuchElementException {
                if (cursor >= toIndex) {
                    throw new NoSuchElementException(InternalUtil.ERROR_MSG_FOR_NO_SUCH_EX);
                }
